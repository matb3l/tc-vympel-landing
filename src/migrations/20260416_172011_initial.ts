...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   CREATE TYPE "public"."enum_contact_submissions_status" AS ENUM('new', 'in_progress', 'done');
  ...
  CREATE TABLE "site_settings" (
  	"id" serial PRIMARY KEY NOT NULL,
  	"company_name" varchar DEFAULT 'ТЦ ВЫМПЕЛ',
  	"phone" varchar DEFAULT '8 495 787-04-76',
  	"phone_secondary" varchar DEFAULT '8 495 660-87-90',
  	"email" varchar DEFAULT 'tcvympel@mail.ru',
  	"address" varchar DEFAULT 'г. Москва, Высоковольтный проезд, д. 13А',
  	"logo_id" integer,
  	"meta_title" varchar,
  ...
  CREATE TABLE "hero_section" (
  	"id" serial PRIMARY KEY NOT NULL,
  	"title" varchar DEFAULT 'Всё для мясопереработки',
  	"subtitle" varchar DEFAULT 'Оболочки, специи, пищевые добавки и технологический инвентарь от надёжного поставщика с 1995 года',
  	"cta_text" varchar DEFAULT 'Получить каталог',
  	"cta_secondary_text" varchar DEFAULT 'Связаться с нами',
  	"background_image_id" integer,
  	"updated_at" timestamp(3) with time zone,
  	"created_at" timestamp(3) with time zone
  );
  
  CREATE TABLE "about_section_advantages" (
  	"_order" integer NOT NULL,
  	"_parent_id" integer NOT NULL,
  	"id" varchar PRIMARY KEY NOT NULL,
  	"title" varchar,
  ...
  CREATE TABLE "about_section" (
  	"id" serial PRIMARY KEY NOT NULL,
  	"title" varchar DEFAULT 'О компании',
  	"description" varchar DEFAULT 'Компания ТЦ ВЫМПЕЛ была основана в 1995 году и на сегодняшний день готова предложить нашим клиентам широкий ассортимент товаров для мясопереработки. С каждым годом у нас увеличивается количество партнёров — нашими поставщиками становятся ведущие компании-производители, расположенные по всему миру.',
  	"image_id" integer,
  	"updated_at" timestamp(3) with time zone,
  	"created_at" timestamp(3) with time zone
  );
  ...
  ALTER TABLE "about_section_advantages" ADD CONSTRAINT "about_section_advantages_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."about_section"("id") ON DELETE cascade ON UPDATE no action;
  ALTER TABLE "about_section" ADD CONSTRAINT "about_section_image_id_media_id_fk" FOREIGN KEY ("image_id") REFERENCES "public"."media"("id") ON DELETE set null ON UPDATE no action;
  ...
  CREATE INDEX "about_section_advantages_order_idx" ON "about_section_advantages" USING btree ("_order");
  CREATE INDEX "about_section_advantages_parent_id_idx" ON "about_section_advantages" USING btree ("_parent_id");
  CREATE INDEX "about_section_image_idx" ON "about_section" USING btree ("image_id");`)
}
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  DROP TABLE "about_section_advantages" CASCADE;
